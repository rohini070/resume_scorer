...
try:
    ...
    logger.error(f"Failed to initialize ResumeScorer: {str(e)}")
    raise
...
class ResumeRequest(BaseModel):
    ...
    def validate_goal(cls, v: str) -> str:
        # Goals are validated against config in the endpoint
        return v.strip()
...
async def score_resume(request: ResumeRequest):
    ...
    try:
        # Get score and skill matches
        score, matched_skills, missing_skills = scorer.evaluate_resume(
            request.resume_text, request.goal
        )
        
        # Generate learning path
        learning_path = scorer.generate_learning_path(missing_skills)
